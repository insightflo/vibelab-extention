...
거버넌스 셋업 완료 후 **자동으로 다음 단계를 안내**합니다:
...
- management/decisions/ADR-001~004.md (Architect)
- design/system/*.md (Designer)
- management/quality-gates.md (QA Manager)
...
대규모 프로젝트이므로 구현 전문가 팀(backend, frontend, test)이 필요합니다.
```
...
```json
{
  "questions": [
    {
      "question": "에이전트 팀을 생성할까요?",
      "header": "다음 단계",
      "options": [
        {
          "label": "⭐ 예, 에이전트 팀 생성 (권장)",
          "description": "/project-bootstrap 실행 - backend, frontend, test 전문가 생성"
        },
        {
          "label": "아니오, 직접 구현 시작",
          "description": "/agile auto 실행 - Claude가 직접 코드 작성"
        },
        {
          "label": "다른 워크플로우 선택",
          "description": "/workflow 실행 - 전체 스킬 목록에서 선택"
        }
      ],
      "multiSelect": false
    }
...
| 선택 | 실행 스킬 |
|------|-----------|
| "예, 에이전트 팀 생성" | `/project-bootstrap` 즉시 실행 |
| "아니오, 직접 구현 시작" | `/agile auto` 즉시 실행 |
| "다른 워크플로우 선택" | `/workflow` 즉시 실행 |
