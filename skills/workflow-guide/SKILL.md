...
> **목적**: 51개 스킬(바이브랩 35개 + Editor-K 6개 + 우리스킬 10개) 중 현재 상황에 가장 적합한 스킬을 **단 하나만** 추천하는 메타 허브입니다.
>
...
| **`/impact`** | `/impact <file>`, "영향도 분석" | 파일 변경 전 의존성/위험도 분석 **(v3.0 NEW)** |
| **`/deps`** | `/deps`, "의존성 그래프" | 순환 감지, Mermaid 시각화 **(v3.0 NEW)** |
| **`/changelog`** | `/changelog`, "변경 이력" | 기간/도메인/유형별 필터링 **(v3.0 NEW)** |
| **`/coverage`** | `/coverage`, "테스트 커버리지" | 미커버 영역, 트렌드 **(v3.0 NEW)** |
| **`/architecture`** | `/architecture`, "아키텍처 맵" | 도메인 구조, API 카탈로그 **(v3.0 NEW)** |
...
**Last Updated**: 2026-02-08 (v3.0.0 - 51개 스킬: 바이브랩 35개 + Editor-K 6개 + 우리스킬 10개)
